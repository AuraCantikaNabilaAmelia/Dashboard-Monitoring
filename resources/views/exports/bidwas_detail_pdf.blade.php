...
<html>
<head>
    <title>Rekap Pegawai Per Bidang</title>
    <style>
        @page { margin: 1.5cm; }
        ...
        .doc-subtitle { font-size: 12px; font-weight: 600; color: #3b82f6; margin-top: 5px; }
        ...
        th { background-color: #1e293b; color: #ffffff; font-weight: 700; text-transform: uppercase; font-size: 9px; padding: 12px 8px; text-align: left; border: none; }
...
<body>
    ...
    <div class="doc-header">
        <div class="doc-title">REKAP PEGAWAI PER BIDANG</div>
        <div class="doc-subtitle">{{ $informasiBidang->nm_bidwas }}</div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="70" class="center">ST Aktif</th>
                <th width="70" class="center">Total ST</th>
            </tr>
        </thead>
        <tbody>
            @php
            ...
            <tr>
                ...
                <td><span class="badge badge-blue">{{ $roleLabels[$pegawai->user_role] ?? $pegawai->user_role }}</span></td>
                <td class="center bold {{ $pegawai->active_tasks > 0 ? 'highlight' : '' }}">{{ $pegawai->active_tasks }}</td>
                <td class="center bold">{{ $pegawai->total_assignments }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
