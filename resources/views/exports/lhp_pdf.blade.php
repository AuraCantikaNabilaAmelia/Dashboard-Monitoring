<!DOCTYPE html>
<html>
<head>
    <title>Daftar Laporan Hasil Pemeriksaan</title>
    <style>
        @page { margin: 1cm; size: landscape; }
        body { font-family: 'Arial', sans-serif; font-size: 10px; color: #1e293b; line-height: 1.4; background-color: #fff; }
        
        .kop-container { position: relative; padding-bottom: 12px; border-bottom: 2pt solid #1e293b; margin-bottom: 2px; }
        .kop-border-thin { border-bottom: 0.5pt solid #1e293b; margin-bottom: 15px; }
        .logo-bpkp { position: absolute; left: 0; top: -5px; width: 70px; }
        .kop-text { text-align: center; margin-left: 20px; }
        .kop-text h1 { font-size: 12px; margin: 0; padding: 0; text-transform: uppercase; color: #1e293b; letter-spacing: 0.5px; }
        .kop-text h2 { font-size: 15px; margin: 2px 0; padding: 0; text-transform: uppercase; font-weight: 800; color: #1e293b; }
        .kop-text p { font-size: 9px; margin: 1px 0 0 0; padding: 0; font-style: italic; color: #64748b; }

        .doc-header { margin-top: 20px; margin-bottom: 15px; text-align: center; }
        .doc-title { font-size: 14px; font-weight: 800; text-transform: uppercase; margin-bottom: 2px; color: #1e293b; border-bottom: 1.5pt solid #3b82f6; display: inline-block; padding-bottom: 2px; }
        
        .doc-metadata { width: 100%; margin-bottom: 10px; font-size: 9px; color: #64748b; }

        table { width: 100%; border-collapse: collapse; margin-top: 5px; table-layout: fixed; }
        th { background-color: #1e293b; color: #ffffff; font-weight: 700; text-transform: uppercase; font-size: 8px; padding: 12px 0 12px 5px; text-align: left; border: none; }
        td { padding: 8px 5px; border-bottom: 0.5pt solid #e2e8f0; vertical-align: top; color: #334155; word-wrap: break-word; }
        tr:nth-child(even) { background-color: #f8fafc; }
        
        .center { text-align: center; }
        .bold { font-weight: 700; }
        .highlight { color: #3b82f6; font-weight: 700; }
        .font-mono { font-family: 'Courier', monospace; font-size: 9px; }
        
        .badge { display: inline-block; padding: 2px 6px; border-radius: 3px; font-size: 8px; font-weight: 700; text-transform: uppercase; }
        .badge-emerald { background-color: #ecfdf5; color: #059669; border: 0.5pt solid #d1fae5; }
        
        .footer { position: fixed; bottom: 0; width: 100%; font-size: 8px; color: #94a3b8; text-align: center; padding-top: 8px; border-top: 0.5pt solid #e2e8f0; }
    </style>
</head>
<body>
    <div class="kop-container">
        <img src="data:image/webp;base64,\" class="logo-bpkp">
    <div class="kop-text">
        <h1>BADAN PENGAWASAN KEUANGAN DAN PEMBANGUNAN</h1>
        <h2>PERWAKILAN PROVINSI JAWA BARAT</h2>
        <p>Jl. Raya Cibeureum No.50, Campaka, Kec. Andir, Kota Bandung, Jawa Barat 40184</p>
    </div>
</div>
<div class="kop-border-thin"></div>

<div class="doc-header">
    <div class="doc-title">LAPORAN HASIL PENGAWASAN (LHP)</div>
</div>

<div class="doc-metadata">
    <table style="width: auto; margin-top: 5px; background: transparent;">
        <tr>
            <td style="padding: 2px 0; border: none; font-weight: 700;">Waktu Cetak</td>
            <td style="padding: 2px 10px; border: none;">:</td>
            <td style="padding: 2px 0; border: none;">{{ date('d-m-Y H:i') }} WIB</td>
        </tr>
    </table>
</div>

<table>
    <thead>
        <tr>
            <th width="30" class="center">No.</th>
            <th width="120">Nomor LHP</th>
            <th width="120">Nomor ST</th>
            <th>Nama Penugasan</th>
            <th width="100">Bidang</th>
            <th width="80" class="center">Tgl. LHP</th>
            <th width="70" class="center">Status</th>
        </tr>
    </thead>
    <tbody>
        @foreach($daftarLhp as $index => $lhp)
        <tr>
            <td class="center">{{ $index + 1 }}</td>
            <td class="bold font-mono">{{ $lhp->no_lhp }}</td>
            <td class="font-mono">{{ $lhp->no_st }}</td>
            <td class="bold">{{ $lhp->nama }}</td>
            <td class="center">{{ $lhp->nm_bidwas ? str_replace('Bidang ', '', $lhp->nm_bidwas) : '-' }}</td>
            <td class="center">{{ date('d-m-Y', strtotime($lhp->tgl_lhp)) }}</td>
            <td class="center">
                <span class="badge badge-emerald">{{ $lhp->status ?: 'TERBIT' }}</span>
            </td>
        </tr>
        @endforeach
    </tbody>
</table>

<div class="footer">
    Dicetak secara otomatis melalui Sistem Monitoring IPMS BPKP Jabar &bull; {{ date('Y') }}
</div>
</body>
</html>
