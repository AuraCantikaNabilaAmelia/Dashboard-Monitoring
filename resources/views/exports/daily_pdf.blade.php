...
<html>
<head>
    <title>Rekap Laporan Harian Pegawai</title>
    <style>
        @page { margin: 1.5cm; }
...
<div class="doc-header">
    <div class="doc-title">REKAP LAPORAN HARIAN PEGAWAI</div>
</div>

<div class="doc-metadata">
    <table style="width: auto; margin-top: 0; background: transparent;">
        <tr>
            <td style="padding: 2px 0; border: none; font-weight: 700;">Tanggal Laporan</td>
            <td style="padding: 2px 10px; border: none;">:</td>
            <td style="padding: 2px 0; border: none;" class="highlight">{{ date('d M Y', strtotime($tanggalLaporan)) }}</td>
        </tr>
        <tr>
...
<table>
    <thead>
        <tr>
            ...
            <th>Nama Pegawai</th>
            <th width="100" class="center">NIP</th>
            <th width="150" class="center">Tanggal</th>
            <th width="100" class="center">Status</th>
        </tr>
    </thead>
    <tbody>
        @foreach($daftarAktivitasHarian as $index => $aktivitas)
        @php
            $warnaStatus = match(strtolower($aktivitas->status_st)) {
                'batal' => 'slate',
        ...
        <tr>
            ...
            <td class="bold">{{ $aktivitas->nama }}</td>
            <td class="center font-mono">{{ $aktivitas->nip }}</td>
            <td class="center bold">{{ date('d M Y', strtotime($aktivitas->tanggal)) }}</td>
            <td class="center">
                <span class="badge badge-{{ $warnaStatus }}">{{ $aktivitas->status_st }}</span>
            </td>
        </tr>
        @endforeach
    </tbody>
