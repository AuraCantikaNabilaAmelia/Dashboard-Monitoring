...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->role === 'kabid') {
            $queryST->where('id_bidwas', $user->bidang_id);
        ...
        */
        ...
        /* SCOPING_BY_BIDANG: Temporarily disabled by user request
        if ($user->role === 'kabid') {
            $activeTasksQuery->where('d_st.id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        */
        if ($user->role === 'pegawai') {
        ...
        if ($user->role === 'kabid') {
            // r_pegawai doesn't have id_bidwas, filtering available employees by bidang is currently limited 
            // to those who have projects in that bidang. Removing to avoid crash.
        } elseif ($user->role === 'pegawai') {
        ...
        /* SCOPING_BY_BIDANG: Temporarily disabled by user request
        if ($user->role === 'kabid') {
            $unfinishedQuery->where('id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        */
        if ($user->role === 'pegawai') {
        ...
        // Global chart data: All roles can see the distribution of all bidwas (as requested)
        ...
        /* SCOPING_BY_BIDANG: Temporarily disabled by user request
        if ($user->role === 'kabid') {
            $monthlyTrend->where('id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        */
        if ($user->role === 'pegawai') {
        ...
        /* SCOPING_BY_BIDANG: Temporarily disabled by user request
        if ($user->role === 'kabid') {
            $statusDist->where('id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        */
        if ($user->role === 'pegawai') {
        ...
        // Global stats stay global but could be scoped for Kabid if needed. 
        // For now, keeping these simple.
    ...
    public function daily(Request $request)
    {
        ...
        /* SCOPING_BY_BIDANG: Temporarily disabled by user request
        if ($user->role === 'kabid') {
            $query->where('d_st.id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        */
        if ($user->role === 'pegawai') {
    ...
    public function stList(Request $request)
    {
        ...
        if ($user->role === 'kabid') {
            $query = DB::table('d_st')
                ->where('d_st.id_bidwas', $user->bidang_id);
        } elseif ($user->role === 'pegawai') {
        }
        */
        if ($user->role === 'pegawai') {
        ...
        $statuses = DB::table('d_st')->distinct()->whereNotNull('status_st')->pluck('status_st');
    ...
    public function lhpList(Request $request)
    {
        ...
        */
        ...
        $statuses = DB::table('d_lhp')->distinct()->whereNotNull('status_lhp')->pluck('status_lhp');
